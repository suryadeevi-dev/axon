...
def put_user(user: dict):
    _table(_TABLE_USERS).put_item(Item=user)


def get_user_by_id(user_id: str) -> Optional[dict]:
    resp = _table(_TABLE_USERS).get_item(Key={"id": user_id})
...
def put_agent(agent: dict):
    _table(_TABLE_AGENTS).put_item(Item=agent)


def get_agent(agent_id: str) -> Optional[dict]:
    resp = _table(_TABLE_AGENTS).get_item(Key={"id": agent_id})
    return resp.get("Item")
...
def delete_agent(agent_id: str):
    _table(_TABLE_AGENTS).delete_item(Key={"id": agent_id})


# ---- Messages ----

def put_message(msg: dict):
    _table(_TABLE_MESSAGES).put_item(Item=msg)
