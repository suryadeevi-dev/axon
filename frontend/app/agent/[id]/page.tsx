...
export default function AgentPage() {
    ...
    switch (event.type) {
      case "token": {
        ...
        setMessages((prev) => {
          ...
          if (last && last.role === "assistant" && last.type === "text" && streamingMsgRef.current === last.id) {
      ...
      case "command": {
        ...
        setMessages((prev) => [
          ...prev,
          { id: `cmd-${Date.now()}`, role: "assistant", type: "command", content: cmd, timestamp: now },
        ]);
      ...
      case "output": {
        ...
        setMessages((prev) => {
          ...
          if (last && last.type === "output") {
            return prev.slice(0, -1).concat({ ...last, content: last.content + out });
          }
      ...
      case "done":
        ...
        break;
      case "error": {
        const errMsg = typeof event.data === "string" ? event.data : "Agent error";
        toast.error(errMsg);
        setSending(false);
        streamingMsgRef.current = null;
        break;
  ...
  const statusColor = wsConnected ? "bg-axon-green" : "bg-red-500";
    ...
    <div className="min-h-screen bg-axon-bg flex flex-col">
      ...
      <div className="border-b border-axon-border px-6 py-3 flex items-center gap-4 bg-axon-surface">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-axon-muted font-mono">
            {agentId.slice(0, 8)}
          </span>
        </div>
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={connectWS}
            className="p-1.5 rounded text-axon-muted hover:text-white transition-colors"
            title="Reconnect"
          >
            <RotateCcw size={12} />
          </button>
        ...
        {(["chat", "terminal"] as Tab[]).map((tab) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium border-b-2 transition-colors ${
              activeTab === tab
                ? "border-axon-cyan text-axon-cyan"
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center pt-12">
                  ...
                  <p className="text-xs text-axon-muted max-w-xs">
                    Ask your agent to run code, install packages, query APIs, or anything a Linux shell can do.
                  </p>
                  <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-sm">
                    {[
                      "Write and run a Python hello world",
                      "Check available disk space",
                      "Clone a GitHub repo and list its files",
              ...
              {sending && !messages.some((m) => m.role === "assistant" && streamingMsgRef.current === m.id) && (
                <div className="flex gap-2 items-center text-axon-muted text-xs animate-pulse">
            ...
            <div className="border-t border-axon-border px-6 py-4">
              <div className="flex items-center gap-3 rounded-xl border border-axon-border bg-axon-surface px-4 py-2.5 focus-within:border-axon-cyan/50 transition-colors">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-axon-cyan text-axon-bg hover:bg-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Send size={14} />
                </button>
              </div>
              <p className="text-[10px] text-axon-muted mt-1.5 pl-1">
          ...
          <div className="flex-1 overflow-y-auto bg-[#050508] p-5 font-mono text-xs leading-relaxed">
            {terminalLines.length === 0 ? (
              <div className="text-axon-muted">
                ...
                <br />
                <span className="text-axon-muted/60">No output yet. Send a message in the Chat tab.</span>
              </div>
            ) : (
              <>
                {terminalLines.map((line, i) => (
                  <div key={i} className={line.startsWith("$") ? "text-axon-cyan mt-2" : "text-axon-green"}>
                    {line}
                  </div>
                ))}
